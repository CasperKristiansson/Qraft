...
it("validates entity length by Unicode code points at the command boundary", () => {
  expect(
    qaCommandSchema.safeParse({ type: "createSection", title: "😀".repeat(2_000) }).success,
  ).toBe(true);
  expect(
    qaCommandSchema.safeParse({ type: "createSection", title: "😀".repeat(2_001) }).success,
  ).toBe(false);
  expect(qaCommandSchema.safeParse({ type: "createSection", title: "bad\0text" }).success).toBe(
    false,
  );
});
...
it("keeps skipped tasks separate from completed progress", () => {
  ...
  copy.sections[0]!.tasks[2]!.status = "skipped";
