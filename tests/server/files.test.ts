...
it("excludes hidden/dependency/symlink paths and revalidates a replaced chosen file", async () => {
  ...
  try {
    ...
    const outside = join(parent, "secret.md"); await writeFile(outside, "outside bytes");
    for (const directory of ["docs", ".hidden", "node_modules", "dist"]) { await mkdir(join(root, directory)); await writeFile(join(root, directory, "Review.md"), "## Section\n"); }
    await symlink(outside, join(root, "link.md"));
    await symlink(parent, join(root, "linked-directory"));
    const catalog = new FileCatalog(root);
    const first = await catalog.list(); expect(first.files.map((file) => file.label)).toEqual(["docs/Review.md"]);
    ...
    await rm(chosen); await symlink(outside, chosen);
    await expect(store.read()).rejects.toMatchObject({ code: "io" });
    await expect(store.execute({ type: "createSection", title: "Escape" }, initial.revision)).rejects.toMatchObject({ code: "io" });
...
it("rejects a symlink substitution at the final revision check", async () => {
  ...
  try {
    const file = join(root, "Review.md"); const victim = join(root, "Other.md");
    await writeFile(file, "# Review\n"); await writeFile(victim, "owner data");
    const store = new MarkdownDocumentStore(file, root, { beforeCommitCheck: async () => { await rm(file); await symlink(victim, file); } });
    await expect(store.execute({ type: "createSection", title: "Added" }, sha256("# Review\n"))).rejects.toMatchObject({ code: "io" });
