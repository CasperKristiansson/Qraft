...
async function readDocument(page: Page): Promise<QADocument> {
  return page.request
    ...
    .then((response) => response.json() as Promise<QADocument>);
}
...
test("M5 picker highlights a host control, suppresses its click, persists only approved context, and fails source-open safely", async ({
  page,
}) => {
  await reset(page);
  await attach(page);
  ...
  await expect(
    drawer.locator(".qraft-context > code").filter({ hasText: /main\.tsx/u }),
  ).toBeVisible();
    ...
    .poll(async () => {
      ...
      return (
        document.sections
          ...
          .find((task) => task.title === "Change quantity")?.notes.length ?? 0
      );
    })
    .toBe(1);
  const document = await readDocument(page);
  const finding = document.sections
    .flatMap((section) => section.tasks)
    .find((task) => task.title === "Change quantity")!.notes[0]!;
  const element = finding.element;
  expect(element).not.toBeNull();
  if (!element?.source)
    throw new Error("Expected the React example target to include a normalized source path.");
  ...
  await page.route("**/__open-in-editor?**", (route) =>
    route.fulfill({ status: 500, body: "blocked" }),
  );
  await page.evaluate(() => {
    window.open = () => {
      window.document.body.dataset.externalEditorFallback = "called";
      return null;
    };
  });
  await drawer.getByRole("button", { name: "Open source" }).click();
  await expect(drawer.getByRole("alert").filter({ hasText: "Could not open" })).toContainText(
    element.source,
  );
  ...
  const editorRequest = page.waitForRequest(
    (request) => new URL(request.url()).pathname === "/__open-in-editor",
  );
  await drawer.getByRole("button", { name: "Open source" }).click();
...
test("M5 picker ignores itself, cancels cleanly, and retains structural context with a plain note fallback", async ({
  page,
}) => {
  ...
  await cancel.click();
  await expect(
    page.getByRole("dialog").getByRole("heading", { name: "Change quantity", exact: true }),
  ).toBeVisible();
  ...
  await expect
    .poll(async () => {
      ...
      return (
        current.sections
          .flatMap((section) => section.tasks)
          .find((candidate) => candidate.title === "Change quantity")?.notes.length ?? 0
      );
    })
    .toBe(1);
  const finalDocument = await readDocument(page);
  const task = finalDocument.sections
    .flatMap((section) => section.tasks)
    .find((candidate) => candidate.title === "Change quantity");
...
test("M5 picker reaches open shadow roots and same-origin iframes", async ({ page }) => {
  ...
  await attach(page);
  await page
    .frameLocator("iframe[title='Same-origin picker target']")
    .getByRole("button", { name: "Iframe quantity" })
    .click();
  await expect(page.getByRole("dialog").getByLabel("Write a note")).toBeVisible();
});
