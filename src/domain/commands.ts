...
const text = z
  ...
  .refine(
    (value) =>
      ...
      !/[\x00-\x08\x0e-\x1f\x7f]/u.test(value),
    "Use 1 to 2,000 Unicode characters without control characters",
  );
...
export const elementContextSchema = z.strictObject({
  ...
  attributes: z
    .record(
      z
        .string()
        .regex(/^(id|class|role|aria-label|name|type|title|data-testid|data-test|data-cy)$/u),
      z.string().max(300),
    )
