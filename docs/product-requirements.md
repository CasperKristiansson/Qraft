...
- Notes can include bounded element identity: route pathname, component, repository-relative source/line/column, React Grab selector, tag, selected identifying attributes, a short visible text excerpt, and a bounded ancestor description. No screenshots, values from inputs, full HTML, styles, Fiber dumps, or arbitrary page capture.
