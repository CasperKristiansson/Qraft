...
Show the task's recognized indented instructions as read-only plain text below its title, preserving line breaks. New notes display their capture pathname and CSS viewport dimensions separately from optional element context. Capture when composition or attachment begins and retain that context through navigation and editing.
