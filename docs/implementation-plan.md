...
## Closed-tab orientation clarification — 2026-09-05

The owner clarified that the compact closed tab should be rotated 90 degrees, vertically along the edge. This supersedes only the horizontal-tab direction and screenshots from the preceding feedback revision. The product/design owners now specify clockwise sideways text and grip, with the concave junctions retained. Position clamping measures the rendered height so longer counts remain contained.

- Base `7ade9fbaddad54e4945086ed27556526a20b578d`; source `a665ea21627649f525c7e592b20338c97508a1c74ecb657557dc71bce9636144`; packed digest `e49107a2f5232979c8078805d976ff1e43948ed7f9c139a2c0df879a9034b38d`. Git started clean; only tab styling/measurement, the existing geometry regression, canonical orientation text and this evidence changed.
- `corepack pnpm check` passed (54 unit/integration tests, typecheck, formatting/lint and build); `corepack pnpm exec playwright test tests/e2e/review-feedback.spec.ts -g 'compact vertical tab'` passed 3/3 engines; `corepack pnpm verify:consumer` passed packed installation/exports/build and five preview-absence probes. These checks ran once; unchanged whole-workflow and license evidence remains inherited from the preceding accepted candidate.
- Packed `@Browser`: `http://127.0.0.1:4185/`, 1440×900, 1366×650 and 768×900. Inspected clockwise rotation, 26×72.15625 px geometry, retained concave joins, opening/closing, unchanged host layout/scroll, no horizontal overflow and keyboard movement/reload persistence (top 429.921875 px before and after reload at 1440×900). Pointer drag and Home/End/resize limits pass in the three-engine regression. Browser warn/error log was empty.
- Native JPEGs: `artifacts/browser-evidence/review/feedback/vertical-tab/closed--{1440x900,1366x650,768x900}.jpg`; per-image hashes and provenance in that directory's `screenshots.json`. All three native headers/dimensions passed inspection. Rapid viewport changes initially returned mismatched screenshot dimensions; captures were repeated only after the browser surface settled, without changing product code. Gallery: `http://127.0.0.1:4181/feedback/vertical-tab/`.
- This is a narrow UI correction, not a new roadmap milestone or a claim of a repeated full acceptance run. No remaining work in this correction; normal commit/push is authorized and hosted CI will not be awaited.


## Pinned checklist actions clarification — 2026-09-05

The owner requested removal of the visible gesture instructions, local Markdown sync/revision and filename footer. Change file now sits beside Qraft in the checklist header; Add section and its form occupy a separate bottom area while the checklist scrolls. The design document owns this updated interaction. No storage, Markdown, dependency or packaging contracts changed.

- Base `db34805d791a140b30c9a0739d3dbe874a02713e`; final source `20ea6a9ea7787701cff42c288ad4c38db4d2e475106d09de72bbece26e260074`; packed digest `eaae9add15bf2a9c1f279ef37fedad940ebd080aaf69a45f0d5a025b2ae4a0bc`. Git started clean. The scoped diff covers the drawer, canonical design and relevant browser regressions; generated evidence and consumer fixture data remain ignored.
- Final `corepack pnpm check` passed (54 unit/integration tests, formatting/lint, typecheck and build). `corepack pnpm test:browser` passed 51/51 across Chromium, Firefox and WebKit. `corepack pnpm verify:consumer` passed clean packed installation, exports/build and five production-absence probes. Receipts: `artifacts/release/pinned-actions-{check,browser,consumer}.log` and `pinned-actions-consumer.json`. Unchanged clean-install and dependency/license acceptance evidence remains inherited from the preceding accepted candidate; this is a narrow UI correction, not a repeated full milestone audit.
- Diagnostic budget: the initial check was blocked only by sandbox loopback `EPERM`, then rerun with local-server permission. The focused 39-browser run exposed detached-button focus restoration after cancel (two failures, 37 passes); stable button IDs now restore focus to the remounted action. Typecheck caught the nullable mount in that fix; it was guarded before the final successful aggregate check. The full browser run validates the corrected behavior; its early cases preceded only that defensive nullable guard. No behavior was weakened and no unchanged gate was repeated without a diagnostic reason.
- The added long-list regression uses 200 extra tasks and verifies scrolling, final-row clearance, pinned action geometry, form controls, focus restoration and no host scrolling/overflow at 1440×900, 1366×650, 768×900 and 768×360. Existing focus-trap checks now follow the new DOM order. External-refresh checks verify the actual refreshed response and a successful save instead of depending on removed revision text.
- Packed `@Browser`: `http://127.0.0.1:4186/` at 1440×900, 1366×650 and 768×900. Chose QA.md, inspected short checklist/header, switched to a 200-task Markdown file, scrolled by keyboard to its last task, opened/canceled the pinned section form, switched back, inspected details and returned. At 768×900, Shift+Tab from Change file reaches Add section and Tab returns to Change file. Cancel returns focus to Add section. No warning/error browser logs.
- The header remains at y=0 with height 58 px. The Add section footer is 69 px tall and ends exactly at each viewport bottom; its expanded form remains inside the 650 px viewport. Captured document/body widths equal the viewport and host scroll remains (0,0). Compared against the approved visual direction and subsequent owner changes: neutral list typography, purple accents and restrained shadow remain, with the requested footer cleanup and persistent quick action.
- Seven native JPEGs and exact viewport/source/artifact/image hashes are recorded in `artifacts/browser-evidence/review/feedback/pinned-actions/screenshots.json`, SHA-256 `525a5b78e63b96f76b2806d8ca7bcd75f5d7788b1b95c25d7e9c8e6d4d61c11e`. All native dimensions/encodings passed inspection. The narrow checklist was recaptured after navigation settled. Gallery: `http://127.0.0.1:4181/feedback/pinned-actions/`.

Remaining work for this clarification: **none**. The owner authorized normal commit/push without waiting for hosted CI. No publish, deployment or new external resource is included.

## Picker refinement — 2026-09-05

The owner authorized the four proposed improvements: reliable tracking/selection, parent navigation, clearer highlights with optional guides, and richer bounded source context. The owning design, architecture, Markdown, protocol, product and acceptance documents now specify these changes. M1–M6 remain complete; earlier evidence is inherited only for unchanged behavior, with changed picker/schema/normalization boundaries revalidated below.

- Base `5445d7657b223fba130230bb27a83a16a67a56db`; final source fingerprint `470f55dd80b4a6c39b29989d89c6817e1baef7726e9611b3f0b857dbb34efc2b`; packed SHA-256 `b54a361821b6cacdce7dce0db9d9c13e0678cafd35f8d034d0f350b045dc51ed`. The repository started clean. All existing work was preserved; the diff contains this refinement, its documentation and tests only.
- Selection uses a separate locked attachment lifecycle, a three-second context timeout, structural fallback, cancellation and a brief selected state. Hover lookups are cached/debounced, geometry and hit testing follow animation frames, and events bind to current/dynamic same-origin documents and open shadow roots. Pointer/mouse activation is suppressed during selection. Parent/child arrows, buttons and an eight-entry element trail hold the target; Resume picking releases it. Focus can preview a host control without pointer input. Faint guides are optional and off by default; dimension labels clamp using their rendered width.
- Optional Context sourceTrail stores at most five deduplicated component/source locations. The client excludes dependency/ignore-listed frames and raw stack arguments; server reads/writes/conflicts normalize every source inside the project and drop outside/dependency paths. Existing Context metadata remains compatible. Full-file goldens cover note creation/editing with the trail, BOM, LF/CRLF and final-newline variants. Source trail length/strict fields, filtering and path containment have focused tests. No new dependency, framework or external service was added.
- Provenance: React Grab repository `aidenybai/react-grab`, inspected commit `ea4bbec9e80f4802e8ae19ad18431edb9ddbb670`; `packages/react-grab/src/primitives.ts`, `utils/create-element-bounds.ts` and `components/overlay-canvas.tsx`, plus installed `react-grab@0.2.0` public declarations. Reused ideas are viewport-coordinate bounds, separate highlight presentation and a bounded projection of structured source frames. Resulting Qraft files are `src/client/picker/ElementPicker.tsx`, `source-trail.ts`, client styles and the domain/Markdown context projection. This is original Qraft composition around public primitives; no upstream subsystem or private import was copied.

### Current acceptance evidence

- Final `corepack pnpm check`: pass, 58 unit/integration tests, formatting/lint, TypeScript and package build. Final `corepack pnpm test:browser`: 63/63 across Chromium, Firefox and WebKit. Final `corepack pnpm audit:release`: pass, 111 installed licenses/notices, exact pins, public exports and browser/Agentation boundaries. Final `corepack pnpm verify:consumer`: pass, 28 archive files, isolated tarball installation, exports/build and five production-absence probes.
- These gates ran in `/var/folders/vh/q11rp8711qz5l2_r6bsndlb00000gn/T/qraft-picker-final-_ia2pgpb`, created from the source manifest including all uncommitted and untracked implementation files, initially without `.git`, dependencies, build output or local QA data. `corepack pnpm install --frozen-lockfile` passed there. Subsequent candidate replacements retained that unchanged frozen dependency graph. The final consumer receipt fingerprint matches the current workspace fingerprint exactly. Logs: `artifacts/release/picker-isolated-{install,check,browser,audit,consumer}.log`; package receipt: `artifacts/release/picker-consumer.json`.
- New browser regressions prove focus/parent/child navigation, held-target movement and label clamping, guide geometry, saved normalized source trail, delayed-click stability, pointerdown suppression, cancellation, timeout fallback, and dynamic scaled-iframe scrolling. Existing picker, file chooser, status, note, conflict, external-refresh, accessibility and production-boundary tests passed unchanged except the locator narrowed for intentionally repeated source locations.
- All 16 acceptance criteria retain current local evidence: criteria 1–8 and 11–15 are covered by the final aggregate tests/package gates and unchanged integrated evidence; picker criteria 9–10 and visual criterion 16 additionally have the fresh packed Browser journey below. No publication, deployment or hosted-CI result is implied.

### Packed Browser journey and visuals

- Final URL `http://127.0.0.1:4188/`; viewports 1440×900, 1366×650 and 768×900. Chose QA.md, opened Change quantity, drafted a note, focused the quantity button, moved outward to the control and whole card, toggled guides, used the element trail to return to the button, attached and saved. Inspected and scrolled expanded source context, resized a held selection, resumed picking, canceled with both the button and Escape, verified retained composer text, selected the host button directly without changing quantity, and edited the saved note while retaining its source trail.
- Host quantity stayed 2 during selection. All captures have document/body widths equal to the viewport and host scroll (0,0). Picker toolbar bounds remain within each viewport, 16 px above its bottom; labels remain beside the selected bounds without overflowing the right edge. Browser warning/error readback was empty. The saved context contains `main.tsx:11:322`, the quantity-button selector, five ancestor descriptions and the App source frame. This small consumer has one relevant frame; bounded multi-frame behavior is proven by the source-trail tests, not fabricated in the fixture.
- Final consumer Markdown SHA-256 after note editing: `9a56e713c9e5352e947e6f02d1795c854596d86315867dac62b77fb087f55d9c`. The original source metadata is retained and the note body reflects the edit. Full-file byte-preservation proof is supplied by the golden tests.
- Seven native Browser JPEGs in `artifacts/browser-evidence/review/feedback/picker-refinement/`; each has URL, exact viewport, source fingerprint, archive digest, geometry, result and image hash in `screenshots.json` (SHA-256 `e9b58b7923e7664bf18168ef9180bbd572e381606a9f27308bd0d00d6b9e1c35`). All native dimensions/encodings passed the screenshot checker and the originals were inspected. Gallery: `http://127.0.0.1:4181/feedback/picker-refinement/`. Diagnostic images from the superseded candidate are separate. The approved neutral typography, restrained purple outline/fill and soft overlay remain; the owner-authorized trail, dimensions and optional guides extend that direction.

### Execution budget and handoff

Focused failures were resolved from their specific evidence: cross-realm Document identity prevented frame event binding; newly inserted frames needed immediate mutation/load binding; existing source-text selectors became ambiguous after the trail added a second location; guide-line visibility and completed-save assertions needed to target the actual rendered/saved result. Type checks caught fixture typing and a timer overload before aggregate gates. Each focused retry followed those corrections.

There was one clean frozen installation and four successful aggregate candidate passes. Three replacements were justified by relevant changes: keyboard-focus preview accessibility, a packed Browser source-trail readability finding, and the narrow-screen fixed-width label-position finding. The last label change also passed its focused three-engine regression before the final aggregate pass. Prior receipts are retained as `picker-before-{keyboard,source-layout,label}-*.log`; no unchanged gate was repeated without a recorded diagnostic or changed boundary.

Remaining implementation/acceptance work for this refinement: **none**. Source identities remain best-effort, not guarantees across application rewrites. The owner authorized normal commit/push without waiting for hosted CI. Build artifacts, local fixture data and screenshot evidence remain ignored; no public release, deployment or external resource creation is included.
