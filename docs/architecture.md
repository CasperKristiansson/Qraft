...
## Shared-backend composition — 2026-09-10

`src/backend.ts` exports `createQraftBackend`, Node HTTP and Lambda HTTP v2 bridges. It accepts
pluggable `QraftBackendStorage` (or an optional persistent filesystem root), stable projectId,
exact public origin, endpoint and mandatory host authorization callback. Authorization precedes
storage access; local Vite/Next entrypoints retain their development-only guards. Safe errors omit
storage paths, bucket details and authorization internals.

`src/s3.ts` exports `createS3Storage`. The optional consumer-owned AWS SDK peer is server-only and
externalized from Qraft bundles. IAM credentials come from the host role. A trusted bucket/prefix
bounds discovery; stable campaign identity yields opaque file IDs across cold starts. The store
reuses command validation, parsing and minimal patches. A full-file revision check followed by
S3 `If-Match` fences concurrent Lambda writers. SDK writes use one attempt, so uncertain outcomes
are never automatically replayed. There are no process-local catalog or lock authorities in S3 mode.
The filesystem alternative retains the existing MarkdownDocumentStore and its recovery limits.
There is no shared SSE endpoint: every poll rechecks current authorization.

`QA.backend` selects the existing HTTP catalog with polling (three seconds by default, minimum
one second), fresh optional host headers and same-origin credentials. Shared endpoints must be
application-relative paths; put a separately hosted backend behind the application's authenticated
reverse proxy. Remount the drawer when endpoint/sessionKey changes. Keep old local session keys
compatible; shared unsaved drafts use an additional endpoint/session namespace. The host must
unmount on logout. Polling reads coalesce and never automatically replay a command. Direct bound
`QAStorage` injection remains compatible and mutually exclusive with `backend`.

A private versioned S3 bucket and one host-authenticated Lambda route provide shared persistence.
No database, container service or filesystem mount is required. Qraft supplies no account provider
or infrastructure deployment. See [shared setup](shared-backend.md) for integration and acceptance.
